...
rime-speed report month                # 最近 30 天（截止昨天，不含今天）
...
> `report` 接受位置参数 `today | yesterday | week | month | YYYY-MM-DD`；省略它则回退到 `--day` / `--from-ts/--to-ts`。`rsp` 是等价简写（`~/.local/bin/rsp` 软链，任意 shell/脚本可用）。
