...
def test_write_lua_creates_file(monkeypatch, tmp_path):
    ...
    dest = rime_typing_speed.write_lua()
    assert dest.exists()
...
def test_ensure_data_dir(monkeypatch, tmp_path):
    ...
    rime_typing_speed.ensure_data_dir()
    assert (tmp_path / "d").is_dir()


def test_patch_schema_custom_creates_and_is_idempotent(monkeypatch, tmp_path):
    monkeypatch.setenv("RIME_DIR", str(tmp_path))
    changed1 = rime_typing_speed.patch_schema_custom(tmp_path, "rime_ice")
    changed2 = rime_typing_speed.patch_schema_custom(tmp_path, "rime_ice")
...
def test_patch_preserves_existing_custom(monkeypatch, tmp_path):
    ...
    rime_typing_speed.patch_schema_custom(tmp_path, "rime_ice")
...
def test_unpatch_removes_only_our_lines(monkeypatch, tmp_path):
    ...
    cust.write_text("patch:\n  menu/page_size: 7\n", encoding="utf-8")
    rime_typing_speed.patch_schema_custom(tmp_path, "rime_ice")
    rime_typing_speed.unpatch_schema_custom(tmp_path, "rime_ice")
    body = cust.read_text(encoding="utf-8")
...
def test_unpatch_deletes_file_it_created(monkeypatch, tmp_path):
    monkeypatch.setenv("RIME_DIR", str(tmp_path))
    rime_typing_speed.patch_schema_custom(tmp_path, "t9")   # creates t9.custom.yaml fresh
    assert (tmp_path / "t9.custom.yaml").exists()
    rime_typing_speed.unpatch_schema_custom(tmp_path, "t9")
