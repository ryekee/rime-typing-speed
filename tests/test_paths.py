...
def test_data_dir_env_override(monkeypatch, tmp_path):
    ...
    assert rime_speed.data_dir() == tmp_path
    assert rime_speed.log_path() == tmp_path / "commits.jsonl"


def test_rime_dir_env_override(monkeypatch, tmp_path):
    monkeypatch.setenv("RIME_DIR", str(tmp_path))
    assert rime_speed.rime_dir() == tmp_path
    assert rime_speed.lua_dest() == tmp_path / "lua" / "speed_logger.lua"
...
def test_embedded_lua_matches_repo_file():
    ...
    assert rime_speed._SPEED_LOGGER_LUA == repo_lua.read_text(encoding="utf-8")
