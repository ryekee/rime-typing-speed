...
def test_parse_valid_line():
    ...
    cm = rime_typing_speed.parse_line(line)
...
def test_parse_blank_and_malformed_returns_none():
    assert rime_typing_speed.parse_line("") is None
    assert rime_typing_speed.parse_line("   ") is None
    assert rime_typing_speed.parse_line("not json") is None
    assert rime_typing_speed.parse_line('{"c":3}') is None  # missing required t


def test_parse_defaults_missing_optional_fields():
    cm = rime_typing_speed.parse_line('{"t":1,"c":2}')
...
def test_read_log_sorts_and_skips_bad(tmp_path):
    ...
    commits = rime_typing_speed.read_log(p)
    assert [c.t for c in commits] == [100, 200]


def test_read_log_missing_file_returns_empty(tmp_path):
    assert rime_typing_speed.read_log(tmp_path / "nope.jsonl") == []
