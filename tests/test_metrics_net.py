...
def test_split_sessions_breaks_on_large_gap():
    ...
    sessions = rime_typing_speed.split_sessions(commits, session_gap=300)
...
def test_active_seconds_and_chars_skips_idle_gaps():
    ...
    secs, chars = rime_typing_speed.active_seconds_and_chars(commits, idle_threshold=5)
    assert secs == 6        # 2 + 2 + 2
...
def test_net_speed_cpm():
    commits = [C(0, 2), C(2, 3), C(4, 2), C(20, 5), C(22, 4)]
    assert rime_typing_speed.net_speed(commits, idle_threshold=5) == 90.0  # 9 / 6 * 60


def test_net_speed_empty_is_zero():
    assert rime_typing_speed.net_speed([]) == 0.0
    assert rime_typing_speed.net_speed([C(0, 5)]) == 0.0
